...
## Table Engagement Stats by Fiscal Year
member_engagement_fiscal <- member_checkin_fiscal %>%
...
member_engagement_fiscal_age <- member_engagement_fiscal %>%
  ...
  mutate(
    Eng_Level = ifelse(checkin_avg_age >= 2, "A. Ideal (2+)", 
                       ifelse(checkin_avg_age >= 1, "B. Typical (1-2)",
                              ifelse(checkin_avg_age > 0, "C. Poor (0-1)",
                                     "D. None (0)")))) %>%
...
## Table Members with Junior Engagement Scores
member_junior_engagement <- member_engagement_fiscal_age %>%
  
  # Return only members with Junior Engagements
  filter(age_category == "J") %>%
  
  # Only want member id
  select(d4g_member_id)
##

## Table: Former Junior Engagement Stats
junior_engagement_fiscal <- member_junior_engagement %>%
  
  # Remove non-junior members
  left_join(member_engagement_fiscal_age, by = "d4g_member_id")
###
...
plot_data <- junior_engagement_fiscal
...
ggplot(plot_data,
  ...
  ggtitle("Junior Engagement Journeys (n = 5481)")
##
###